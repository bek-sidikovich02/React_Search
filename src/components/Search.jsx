...
const Search = () => {
  ...
  useEffect(() => {
    try {
      axios
        .get(
          `https://api.spaceflightnewsapi.net/v3/articles/volumes?q=${text}&orderBy=${orderBy}&startIndex=${startIndex}&maxResults=${maxResult}`
        )
        .then((res) => {
    ...
    try {
      for (let i = 0; i < data.length; i++) {
        if (data[i].volumeInfo.categories[0].includes(category) === true) {
          ...
          setresulttitle(`Founded atricles : ${realData.length} article(s)`);
        } else if(category==='All'){
          realData.push(data[i]);
          setresulttitle(`Founded articles : ${realData.length} article(s)`);
          console.log("category is all");
        }else {
          realData.length === 0
            ? setresulttitle("no articles found with this category")
            : setresulttitle("");
        }
      }
    } catch (error) {
